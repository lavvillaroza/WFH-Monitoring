...
export async function GET() {
  try {
    ...
    const latestDTRP = await prisma.dailyTimeRecordProblem.findFirst({
      ...
      select: { id: true, createdAt: true, status: true, employeeId: true },
    });

    const latestLeave = await prisma.leave.findFirst({
      orderBy: { createdAt: "desc" },
      select: { id: true, createdAt: true, status: true, employeeId: true }, // Include employeeId
    });
    ...
    return NextResponse.json({
      ...
      pending: [
        ...pendingDTRP.map((req) => ({ type: "DTRP", ...req })),
        ...pendingLeave.map((req) => ({ type: "Leave", ...req })),
      ],
    });
  } catch (error) {
